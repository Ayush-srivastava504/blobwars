...
const GUN_DISPLAY_W = 96; // bumped up again from 66 per request for even bigger, more visible guns
...
export class ArenaScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Moon cave tileset: frame 0 = seamless floor tile, frame 1 = ledge/edge tile.
    this.load.spritesheet("tiles-moon-cave", "/assets/tiles/moon-cave-tileset.png", {
      frameWidth: 64,
      frameHeight: 64,
    });
  ...
  private drawGrid() {
    // Moon cave floor tileset, tiled across the whole arena, drawn behind everything else.
    const floor = this.add.tileSprite(0, 0, WORLD.WIDTH, WORLD.HEIGHT, "tiles-moon-cave", 0);
    floor.setOrigin(0, 0);
    floor.setDepth(-100);

    const g = this.add.graphics();
    g.lineStyle(1, 0x1c2233, 0.35);
    const step = 100;
